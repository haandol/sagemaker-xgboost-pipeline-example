...
def handler(event, context):
    ...
    job_name = f'dongkyl-train-{job_uniq_id}'
    endpoint_name = f'dongkyl-endpoint-{job_uniq_id}'
    ...
    boto3.Session().resource('s3').Bucket(bucket).Object(os.path.join(prefix, 'train/train.csv')).upload_file('/tmp/train.csv')
    boto3.Session().resource('s3').Bucket(bucket).Object(os.path.join(prefix, 'validation/validation.csv')).upload_file('/tmp/validation.csv')
